...
module.exports = async (client, message) => {
    ...
    let prefix = [`<@!${client.user.id}> `,`sw!`].find(p => message.content.toLowerCase().indexOf(p) === 0)
    ...
    if (level < client.levelCache[cmd.conf.permLevel]) return client.global.message.error(message, "level_not_met", `You do not have permission to run this.`, "(LEVEL_NOT_MET)")
    ...
    //if (client.global.getGuild(message.guild.id).channel === null) {
        client.global.log.log(`${client.config.permLevels.find(l => l.level === level).name} ${message.author.username} (${message.author.id}) ran command [${cmd.help.name}] in (${message.guild.id})`);
