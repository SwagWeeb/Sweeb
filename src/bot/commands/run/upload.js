exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
    ...
    let data = args[2].toLowerCase() === "true";
    if (typeof data !== "boolean") return client.global.message.error(message, "not_bool", "Not a boolean! for NSFW tag use \`true/false\`!", "(NOT_A_BOOLEAN)");
    const date = new Date();
    client.global.db.query(`INSERT INTO 
    sweebData 
    (id, category, nsfw, date, fileLink) 
    VALUES (${client.global.escapeDB(client.global.createID(30))}, ${client.global.escapeDB(args[0].toProperCase())}, ${client.global.escapeDB(data)}, ${date}, ${client.global.escapeDB(args[1])})`)
